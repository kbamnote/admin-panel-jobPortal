      ...
      try {
        ...
        if (userRole === 'eliteTeam') {
          ...
          response = await allJobs(
            ...
            'not verified', // Always show not verified jobs
      ...
      <div className="bg-[var(--color-white)] p-6 rounded-xl shadow-lg">
        ...
        {userRole === 'admin' && (
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-[var(--color-text-primary)]">Not Verified Jobs</h2>
  ...
  const getFilterTitle = () => {
    ...
    if (userRole === 'eliteTeam') {
      if (selectedTeamMember) {
        ...
        return member ? `${member.name}'s Not Verified Jobs` : 'Team Member Not Verified Jobs';
      }
      return 'All Not Verified Jobs';
    }
    
    switch (filterType) {
      case 'admin':
        return 'Admin Posted Not Verified Jobs';
      case 'teamMember':
        if (selectedTeamMember) {
          const member = teamMembers.find(m => m._id === selectedTeamMember);
          return member ? `${member.name}'s Not Verified Jobs` : 'Team Member Not Verified Jobs';
        }
        return 'Team Member Not Verified Jobs';
      default:
        return 'All Not Verified Jobs';
    }
  };
    ...
    <div className="bg-[var(--color-white)] p-6 rounded-xl shadow-lg">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
        ...
        <div className="text-sm text-[var(--color-text-muted)]">
          {totalJobs} not verified {totalJobs === 1 ? 'job' : 'jobs'} found
        </div>
      </div>
      ...
      {jobs.length === 0 ? (
        <div className="text-center py-16 bg-[var(--color-background-light)] rounded-xl">
          ...
          <h3 className="text-xl font-semibold text-[var(--color-text-secondary)] mb-2">No not verified jobs found</h3>
          <p className="text-[var(--color-text-muted)]">
            {userRole === 'admin' ? (
              filterType === 'admin' 
                ? 'There are currently no not verified jobs posted by admin.' 
                : filterType === 'teamMember' 
                  ? (selectedTeamMember ? 'This team member has not posted any not verified jobs yet.' : 'Please select a team member to view their not verified jobs.')
                  : 'There are currently no not verified job postings available.'
            ) : (
              'There are currently no not verified job postings available.'
            )}
          </p>
...
export default NotVerifiedJobs;