...
import { MapPin, Building, Clock, DollarSign, Calendar, Eye } from 'lucide-react';
...
const TeamPostedJobs = () => {
  ...
  const fetchJobsByTeamMember = async (teamMemberId) => {
    if (!teamMemberId) return;
    
    try {
      setJobsLoading(true);
      setJobsError(null);
      const response = await jobsByTeamMember(teamMemberId);
      if (response.data.success) {
  ...
  const handleTeamMemberChange = (e) => {
    ...
    if (teamMemberId) {
      fetchJobsByTeamMember(teamMemberId);
    }
  };
